...
public class VanillaFallFeature implements FallFeature, RegistrableFeature {
	...
	protected Point getLandingPos(LivingEntity livingEntity, Pos position) {
		...
		var fences = Block.staticRegistry().getTag(Key.key("fences"));
		var walls = Block.staticRegistry().getTag(Key.key("walls"));
		var fenceGates = Block.staticRegistry().getTag(Key.key("fence_gates"));
		
		assert fences != null;
		assert walls != null;
		assert fenceGates != null;
		
		if (fences.contains(RegistryKey.unsafeOf(block.key()))
				|| walls.contains(RegistryKey.unsafeOf(block.key()))
				|| fenceGates.contains(RegistryKey.unsafeOf(block.key()))) {
			return offsetDown;
		}
